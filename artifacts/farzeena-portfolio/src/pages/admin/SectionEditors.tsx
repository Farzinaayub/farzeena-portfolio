      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 border shadow-sm space-y-5">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="font-bold text-base text-navy">Showcase Designs</h3>
              <p className="text-xs text-slate-400 mt-0.5">Power BI / Tableau report cards shown in the hero. Leave Image URL empty to use a built-in placeholder design.</p>
            </div>
            <button
              type="button"
              onClick={() => setFormData(p => ({ ...p, showcaseDesigns: [...(p.showcaseDesigns||[]), { title: '', description: '', imageUrl: '', tag: 'Power BI', order: (p.showcaseDesigns?.length||0) }] }))}
          ...
          <div className="space-y-4">
            {formData.showcaseDesigns?.map((design, idx) => (
              <div key={idx} className="border border-slate-200 rounded-xl p-4 bg-slate-50 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-bold text-slate-600 uppercase tracking-wide">Design {idx + 1}</span>
                  <button type="button" onClick={() => setFormData(p => ({ ...p, showcaseDesigns: p.showcaseDesigns?.filter((_, i) => i !== idx) }))} className="p-1.5 text-red-400 hover:bg-red-50 rounded-lg transition-colors">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" value={design.title} onChange={e => { const d=[...(formData.showcaseDesigns||[])]; d[idx]={...d[idx],title:e.target.value}; setFormData(p=>({...p,showcaseDesigns:d})); }} placeholder="Revenue Leakage Analysis" className="w-full px-3 py-2 border rounded-lg text-sm mt-1 focus:ring-2 focus:ring-primary/20" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600">Tag</label>
                    <input type="text" value={design.tag||''} onChange={e => { const d=[...(formData.showcaseDesigns||[])]; d[idx]={...d[idx],tag:e.target.value}; setFormData(p=>({...p,showcaseDesigns:d})); }} placeholder="Power BI" className="w-full px-3 py-2 border rounded-lg text-sm mt-1 focus:ring-2 focus:ring-primary/20" />
                  </div>
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-600">Description</label>
                  <input type="text" value={design.description||''} onChange={e => { const d=[...(formData.showcaseDesigns||[])]; d[idx]={...d[idx],description:e.target.value}; setFormData(p=>({...p,showcaseDesigns:d})); }} placeholder="Short description of what this dashboard shows" className="w-full px-3 py-2 border rounded-lg text-sm mt-1 focus:ring-2 focus:ring-primary/20" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-600 flex items-center gap-1"><Image className="w-3 h-3" /> Image URL (optional)</label>
                  <input type="text" value={design.imageUrl||''} onChange={e => { const d=[...(formData.showcaseDesigns||[])]; d[idx]={...d[idx],imageUrl:e.target.value}; setFormData(p=>({...p,showcaseDesigns:d})); }} placeholder="https://drive.google.com/... or direct image URL" className="w-full px-3 py-2 border rounded-lg text-sm mt-1 focus:ring-2 focus:ring-primary/20" />
                  <p className="text-[10px] text-slate-400 mt-1">If empty, a built-in placeholder chart is shown instead.</p>
                </div>
              </div>
            ))}
            {(!formData.showcaseDesigns || formData.showcaseDesigns.length === 0) && (
              <div className="text-center py-6 border-2 border-dashed border-slate-200 rounded-xl">
                <p className="text-slate-400 text-sm">No designs added yet.</p>
                <p className="text-slate-400 text-xs mt-1">Built-in placeholder designs will be shown in the hero.</p>
              </div>
            )}
          </div>
        </div>
